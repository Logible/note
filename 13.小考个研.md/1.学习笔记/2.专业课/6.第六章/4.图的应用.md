...
假设从顶点0出发，即$V_0=0$, 集合S最初只包含顶点$v_0$

1. 初始化：集合$S$初始为$v_0$, $dist[]$ 的初始值$\displaystyle dist[i]=arcs[0] [i], i=1, 2, ..., n-1$
...
- 从源点到汇点的有向路径可能有多条，所有路径中，具有**最大路径长度的路径**称为关键路径，而把关键路径上的活动称为关键活动
