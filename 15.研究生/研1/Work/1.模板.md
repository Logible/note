...
sudo scp -P 56000 -r /data/log/DaGAN/CVPR2022-DaGAN/emoca_test.png __su@10.112.226.54:/data/log/DaGAN_Serve/test_files/
...
cp /data/log/emoca/assets/* /data/log/emoca_partcode/assets/
...
for dir in /data/vox/*/; do rm "$(find "$dir" -maxdepth 1 -type f -name '*.png' | sort -V | tail -n 1)"; done

ls -d /data9/log/video-preprocessing/vox/train | head -n 2

/data9/log/video-preprocessing/vox/train/id10001#7w0IBEWc9Qw#000993#001143.mp4
/data9/log/video-preprocessing/vox/train/id10001#7w0IBEWc9Qw#001298#001705.mp4

for dir train/*; do sudo rm -rf "$(find "$dir" -maxdepth 1 -type f -name '*.png' | sort -V | tail -n 1)"; done

python load_videos.py --metadata vox-metadata.csv --format .mp4 --out_folder vox2 --workers 20

conda-pack -n video_env -o /data9/log/video_process.tar.gz

CUDA_VISIBLE_DEVICES=3 python run_dataparallel.py --config config/vox_serve.yaml --device_ids 0 --name EaGAN_model --rgbd --batchsize 20 --kp_num 15 --generator DepthAwareGenerator

--image_shape, --iou_with_initial,--min_frames,--max_frames

tar -xzf video_process.tar.gz -C /home/__su/anaconda3/envs/video_process

tar -czvf EaGAN.tar.gz /data2/log/Ea132

cd /data/log/DaGAN_Serve/checkpoint/

## 0409

for dir in test/*; do sudo rm -rf "$(find "$dir" -maxdepth 1 -type f -name '*.png' | sort -V | tail -n 1)"; done

/data2/log/Ea132/log/vox_serveEaGAN_model/reconstruction/id10280#NXjT3732Ekg#001093#001192.mp4.mp4

CUDA_VISIBLE_DEVICES=3 python run_dataparallel.py --config config/vox_serve.yaml --device_ids 0 --name EaGAN_model --rgbd --batchsize 18 --kp_num 15 --generator DepthAwareGenerator --checkpoint /data9/log/Ea132/log/vox_serveEaGAN_model/00000009-checkpoint.pth.tar

## 0410

conda create --name evaluation

conda create --name CSIM python=3.8

conda-pack --force -n metric_env -o /data9/log/metric_env.tar.gz

tar -xzf video_process.tar.gz -C /home/__su/anaconda3/envs/metric_env

cp /data/anaconda3/envs/metric_env /data9/log/metric_env

cp -r metric_env /home/__su/anaconda3/envs/

conda activate /data2/log/metric_env/metric_env

sudo scp -P 56000 -r /data/log/pose-evaluation/* __su@10.112.226.54:/data2/log/

sudo chmod -R 777 /data2/log/metric_env

/data2/log/metric_env/metric_env/bin/python -m pip install numpy

conda-pack -n metics_env -o /data2/log/metric_env.tar.gz

conda config --remove envs_dirs /data2/log/metric_env/metric_env

conda config --append envs_dirs /home/__su/anaconda3/envs/

sudo chmod -R 777 /data2/log/pose-evaluation

sudo scp -P 22 -r /data9/log/video-preprocessing.tar.gz log@WINDOWS-8VKNOKI:/mnt/h/vox

scp -P 56000 __su@10.112.226.54:/data2/log/video-preprocessing.tar.gz /mnt/h/vox/

## 0411

python extract.py --in_folder /data9/log/pose-evaluation/testData --out_file pose_gt.pkl --is_video --type face_pose --image_shape 256,256

cp /data2/log/metric_env/metric_env/* /home/__su/anaconda3/envs/metirc_54/

tar -czvf Ea132.tar.gz -C /data2/log Ea132

scp -P 56000 __su@10.112.226.54:/data2/log/EaGAN4090.tar.gz /mnt/h/vox/

tar -czvf 4090.tar.gz -C /data2/log/EaGAN/

tar -czvf EaGAN4090.tar.gz -C /data2/log EaGAN

## 0412

tar -czf /data9/log/video-processing.tar.gz video-processing

文件：video-processing.tar.gz
环境: video_process.tar.gz

scp -P 56000 __su@10.112.226.54:/data2/log/video-processing.tar.gz /mnt/h/vox/
scp -P 56000 __su@10.112.226.54:/data2/log/video_process.tar.gz /mnt/h/vox/

爬虫的脚本和视频网站

我需要爬取高清，电影电视剧人物片段(个人学习使用，不涉及商业使用)，有哪些爬虫的脚本值得推荐

mv /data/anaconda3/envs/ArcFace /data/anaconda3/envs/modelscope

conda create --name ArcFace --clone modelscope

sudo chmod -R 777 /data9/log/

## 0415

/data/anaconda3/envs/ArcFace

/data9/log/InsightFace_Pytorch
...
- perceptual - 95.33218;
- **gen_gan** - 0.61432;
- **feature_matching** - 4.14658;
- **equivariance_value** - 0.34019;
- equivariance_jacobian - 0.55428;
- kp_distance - 3.24997;
- **disc_gan** - 0.25488

ls -l /proc/4007922/exe

## 0418

FaceLandmarkImg.exe -f "G:\dataset\landmark\muct\jpg\i000qb-fn.jpg" -aus -pose

.\FaceLandmarkImg.exe -fdir "G:\dataset\landmark\muct\jpg" -aus -pose

## 0419

--config config/vox_serve.yaml --device_ids 0 --name DaGAN_model --mode reconstruction --rgbd --batchsize 8 --kp_num 15 --generator DepthAwareGenerator --checkpoint /data2/log/Ea132/log/vox_serveEaGAN_model/00000024-checkpoint.pth.tar
...
python run_dataparallel.py --config config/vox_serve.yaml --device_ids 0 --name DaGAN_model --rgbd --batchsize 8 --kp_num 15 --generator DepthAwareGenerator --checkpoint /data2/log_bat/Ea132/log/vox_serveEaGAN_model/00000024-checkpoint.pth.tar

python run_dataparallel.py --config config/vox_serve.yaml --device_ids 0 --name DaGAN_Orinal --batchsize 1 --rgbd --kp_num 15 --generator DepthAwareGenerator --checkpoint /data2/log_bat/DaGAN_vox_adv_256.pth.tar --mode reconstruction
